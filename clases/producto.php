...
class Producto {
    ...
    function mostrarPorId($idproducto){
        $sql="SELECT * FROM producto p LEFT JOIN foto f ON f.fkproducto=p.idproducto WHERE
    ...
    function agregarCarrito($fkventa, $fkproducto, $cantidad, $subtotal){
        $sql="INSERT INTO detalle_venta(iddetalle_venta , fkventa, fkproducto, cantidad, subtotal, estatus )  VALUES(NULL, '{$fkventa}','{$fkproducto}','{$cantidad}','{$subtotal}', 0)";
        $respuesta=$this->conexion->query($sql);
